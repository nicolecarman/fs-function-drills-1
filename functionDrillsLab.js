...
function greeting(name) {
  // ensures whatever is entered as "name" is converted to string
  name = String(name);
...
function add(number1, number2) {
  // ensures whatever is entered as "number1" or "number2" is converted to an integer
  number1 = +number1;
  number2 = +number2;
...
let nameGreeting = nameCheck("Nicole");
...
let bestMovieChecker = bestMovie("Harry Potter and the Deathly Hallows");

console.log(bestMovieChecker);
...
function emailCheck(email) {
  // converts email to string and trims off excess whitespace
  email = String(email.trim());

  // checks to make sure the email contains @ symbol
  if (email.includes("@")) {
...
let userEmail = emailCheck("nicolecaran90@outlook.com");
...
let totalFrogs2 = buyChocolateFrogs2(8);
